...
is_executing_process_empty = False
def assignation():
    # Window config
    window = ttk.Window()
...
def counter(window, global_counter, global_counter_container,executing_process,ready_process,new_process_title,finished_process, finished_process_list, blocked_processes):
    ...
    global new_list
    global blocked_list
    global is_executing_process_empty
    ...
    if not is_paused:
        if is_interrupted and (len(ready_list) > 0):
            if process_to_show.null_process == False:
            ...
            is_executing_process_empty = True
            if process_to_show.null_process == False:
                blocked_list.append(process_to_show)
                process_to_show = Process('-',0,0,0,0)
